...
class Seg_ver_aplicacionModel extends ModeloBasePDO {
public function __construct() { 
parent::__construct(); 
} 
 public function findall() { 
 $sql = " SELECT id,nro,obs,cod_apl,cod_apl_sup,cod_mod,dsc_apl,obs_apl,tpo_apl,menu,ayuda,url,target_tpo,hr_ini,ht_fin,sw_vie,sw_sab,sw_dom,sw_fer,estado,usu_cre,fh_cre,usu_mod,fh_mod
 FROM seg_ver_aplicacion ";
 $param = array(); 
 ...
 $param = array(); 
  ...
  $var =  parent::gselect($sql, $param);
         $sqlCount = "SELECT count(1) as cant FROM seg_ver_aplicacion 
      WHERE  upper(concat(IFNULL(id,''),IFNULL(nro,''),IFNULL(obs,''),IFNULL(cod_apl,''),IFNULL(cod_apl_sup,''),IFNULL(cod_mod,''),IFNULL(dsc_apl,''),IFNULL(obs_apl,''),IFNULL(tpo_apl,''),IFNULL(menu,''),IFNULL(ayuda,''),IFNULL(url,''),IFNULL(target_tpo,''),IFNULL(hr_ini,''),IFNULL(ht_fin,''),IFNULL(sw_vie,''),IFNULL(sw_sab,''),IFNULL(sw_dom,''),IFNULL(sw_fer,''),IFNULL(estado,''),IFNULL(usu_cre,''),IFNULL(fh_cre,''),IFNULL(usu_mod,''),IFNULL(fh_mod,''))) like CONCAT('%',upper(IFNULL(:p_filtro,'' )), '%') ";
 							        $param = array();
        array_push($param, [':p_filtro', $p_filtro, PDO::PARAM_STR]);
...
function findId($p_id) { $sql = " SELECT id,nro,obs,cod_apl,cod_apl_sup,cod_mod,dsc_apl,obs_apl,tpo_apl,menu,ayuda,url,target_tpo,hr_ini,ht_fin,sw_vie,sw_sab,sw_dom,sw_fer,estado,usu_cre,fh_cre,usu_mod,fh_mod
 FROM seg_ver_aplicacion
  WHERE id = :p_id";
 $param = array(); 
...
public function insert($p_id,$p_nro,$p_obs,$p_cod_apl,$p_cod_apl_sup,$p_cod_mod,$p_dsc_apl,$p_obs_apl,$p_tpo_apl,$p_menu,$p_ayuda,$p_url,$p_target_tpo,$p_hr_ini,$p_ht_fin,$p_sw_vie,$p_sw_sab,$p_sw_dom,$p_sw_fer,$p_estado,$p_usu_cre,$p_fh_cre,$p_usu_mod,$p_fh_mod) {
 $sql = " INSERT INTO seg_ver_aplicacion
 (id,nro,obs,cod_apl,cod_apl_sup,cod_mod,dsc_apl,obs_apl,tpo_apl,menu,ayuda,url,target_tpo,hr_ini,ht_fin,sw_vie,sw_sab,sw_dom,sw_fer,estado,usu_cre,fh_cre,usu_mod,fh_mod) 
 VALUES (:p_id,:p_nro,:p_obs,:p_cod_apl,:p_cod_apl_sup,:p_cod_mod,:p_dsc_apl,:p_obs_apl,:p_tpo_apl,:p_menu,:p_ayuda,:p_url,:p_target_tpo,:p_hr_ini,:p_ht_fin,:p_sw_vie,:p_sw_sab,:p_sw_dom,:p_sw_fer,:p_estado,:p_usu_cre,:p_fh_cre,:p_usu_mod,:p_fh_mod) ";
 $param = array(); 
array_push($param, [':p_id', $p_id, PDO::PARAM_INT]);
...
function delete($p_id) { $sql = "DELETE FROM seg_ver_aplicacion WHERE id = :p_id"; $param = array(); 
array_push($param, [':p_id', $p_id, PDO::PARAM_INT]);
